-- Owner-controlled delivery-cost allocation.
-- The existing delivery_fee_cents remains the authoritative fee. These
-- columns decide only how that fee is allocated between restaurant/customer.
set check_function_bodies = off;

alter table public.tenant_settings
  add column if not exists delivery_cost_mode text not null default 'customer',
  add column if not exists delivery_customer_share_percent integer not null default 100;

alter table public.tenant_settings
  drop constraint if exists tenant_settings_delivery_cost_mode_chk,
  drop constraint if exists tenant_settings_delivery_customer_share_chk;

alter table public.tenant_settings
  add constraint tenant_settings_delivery_cost_mode_chk
    check (delivery_cost_mode in ('restaurant', 'customer', 'split')),
  add constraint tenant_settings_delivery_customer_share_chk
    check (delivery_customer_share_percent between 0 and 100);
...
begin
  ...
  if new.tech_fee_enabled is distinct from old.tech_fee_enabled
     ...
     or new.delivery_cost_mode is distinct from old.delivery_cost_mode
     or new.delivery_customer_share_percent is distinct from old.delivery_customer_share_percent
...
declare
  ...
  v_customer_delivery_fee integer;
  v_restaurant_delivery_fee integer;
  v_tech_fee        integer;
  v_total           integer;
...
begin
  ...
  -- The configured fee remains the single source of truth. Only its payer
  -- changes, with deterministic cent rounding for split allocations.
  v_customer_delivery_fee := case v_settings.delivery_cost_mode
    when 'restaurant' then 0
    when 'split' then round(v_delivery_fee * v_settings.delivery_customer_share_percent / 100.0)::integer
    else v_delivery_fee
  end;
  v_restaurant_delivery_fee := v_delivery_fee - v_customer_delivery_fee;
  ...
  v_total := v_subtotal + v_tax + v_tip + v_customer_delivery_fee + v_service_fee + v_tech_fee;
  ...
  return jsonb_build_object(
    ...
    'taxCents',         v_tax,
    'tipCents',         v_tip,
    'deliveryFeeCents', v_customer_delivery_fee,
    'deliveryFeeTotalCents', v_delivery_fee,
    'deliveryFeeRestaurantCents', v_restaurant_delivery_fee,
    'deliveryFeeCustomerCents', v_customer_delivery_fee,
